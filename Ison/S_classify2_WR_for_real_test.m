function [predicted_class_time_squence,all_output,av_predictedOutput, success_rate, av_confidence_all,std_confidence_all, av_max_conf, std_max_conf, error, error2, confidence_matrix] = S_classify2_WR_for_real_test(predictedOutput, nsteps,tw, proc, nOutputUnits)
...
%     nsteps = floor(length(predictedOutput{i})/time_window);
    av_predictedOutput{i} = zeros(nsteps+1,nOutputUnits);
    av_predictedOutput{i}(1,:) = [1 zeros([1,nOutputUnits-1])];
    for j=1:nsteps-1
...
%      [val, ind] = max(sum_av_predictedOutput );
    [val, ind] = max(sum_av_predictedOutput./sum(sum_av_predictedOutput));
     ...
     ss = sort(sum_av_predictedOutput./sum(sum_av_predictedOutput)); %% sort mean from small to big re-range data
     confidence = ss(end)-ss(end-1);
    
     predicted_class = zeros(1,nOutputUnits);
     predicted_class(ind) = 1;
     
     predicted_class_time_squence{i}=predicted_class;
     
     if sum(predicted_class == av_predictedOutput{i}(1,:)) ==nOutputUnits
