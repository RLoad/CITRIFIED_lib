function varargout = importCAPSData(fileName)
...
if exist(daqFileName, 'file')
    [daq, settings.DAQ_FRAME, settings.DAQ_FRINC, settings.DAQ_SAMP, settings.numChannels] = readDAQfile(daqFileName);
end
...
%% HELPER FUNCTIONS
function [daq, DAQ_FRAME, DAQ_FRINC, DAQ_SAMP, numChannels] = readDAQfile(fileName)
    ...
    for var = 1:nVars
        ...
        try
            if size(dummy,1)==1 % row vector or scalar
                pvd.(varList{var})(idx2Use,:)   = repmat(dummy, DAQ_FRINC, 1);
            elseif size(dummy,2)==1 % column vector
                pvd.(varList{var})(idx2Use,:)   = repmat(dummy', DAQ_FRINC, 1);
            else
                pvd.(varList{var})(idx2Use, :,:) = permute(repmat(PCEvarfilemex(8, varList{var}), [1 1 DAQ_FRINC]), [3 1 2]);
            end
        catch
