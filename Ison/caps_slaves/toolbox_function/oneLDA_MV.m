function [scores,results_per_grasp,LDAmodels,maxValue]=oneLDA_MV(trSet,trSetLabels,div,historyTW,startTW,DiscrimType)
...
for j=1:size(trainingDataSet,2)
        ...
        trainingDataSet(:,j)=trainingDataSet(:,j)/maxValue(j);
...
LDAmodels=fitcdiscr(trainingDataSet,trainlabels,'DiscrimType',DiscrimType);
            
[predictlabels,scoreLDA,costLDA]=predict(LDAmodels,trainingDataSet);
    
ss=100*sum(predictlabels==trainlabels)/length(trainlabels);
    
qerror=resubLoss(LDAmodels);
    
cvmodel = crossval(LDAmodels,'kfold',4);
    
%     cverror1= kfoldLoss(cvmodel);
    
cvIndividualSuccessRate = 100-100*kfoldLoss(cvmodel,'mode','individual');

disp(['CV: ' num2str(mean(cvIndividualSuccessRate)) ' +- ' num2str(std(cvIndividualSuccessRate))])
    
scores.totalSuccessRate=ss;
scores.crossvalidationPerf=mean(cvIndividualSuccessRate);
scores.crossvalidationPerfSTD=std(cvIndividualSuccessRate);
...
%     end
    ...
    [predictlabels(:,i),scoreLDA,costLDA]=predict(LDAmodels,trSet{i});
    
    ss=100*sum(predictlabels(:,i)==trSetLabels)/length(trSetLabels);
