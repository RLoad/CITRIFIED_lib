function [scores,results_per_grasp,SVMmodels]=oneSVM_MV(trSet,trSetLabels,validSet,validSetLabels,div,historyTW,gh,ch,startTW,kernelType)
...
SVMmodels=svmtrain(trainlabels, trainingDataSet, [' -s 0 -t ' num2str(kernelType) ' -g ' num2str(gh) ' -c ' num2str(ch)]);

for i=1:div
    
    for j=1:size(validSet{i},2)
    ...
    %# train one-against-all models
   % SVMmodels{i} = svmtrain(trSetLabels, trSet{i}, [' -s 0 -t 2 -g ' num2str(gh) ' -c ' num2str(ch)]);
    
    
    %# get probability estimates of test instances using each model
    [resultsTrain(:,i), TrainAccuracy, TrianDecision] = svmpredict(trSetLabels, trSet{i}, SVMmodels);

    
    [resultsValid(:,i), ValidationAccuracy, ValidationDecision] = svmpredict(validSetLabels, validSet{i}, SVMmodels);
