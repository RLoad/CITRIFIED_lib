...
function [dataFolder1,dataFolder2,dataFolder3,testingSet,FdataFolder1,FdataFolder2,FdataFolder3,FtestingSet]=createDataSets_byAngle(SubjectData,ClasseIDs,nbTW,sbjID,varargin)
...
for Fnumber=1:nbCrossValidationFolders-1
...
%     
%     for i=1:length(ClasseIDs)
%         lnts=[lnts;length(SubjectData.grasp{ClasseIDs(i)}.trial)];
%     end

%     if Fnumber==nbCrossValidationFolders
%         ranT=1:length(SubjectData.grasp{ClasseIDs(1)}.trial);
%     else
%    
%         ranT=random_numbers(length(SubjectData.grasp{ClasseIDs(1)}.trial),trialsPerFolder,1);
% 
%     end
...
%         tmpFolder2=data2{i}(ranT);
%         tmpFolder3=data3{i}(ranT);
...
%                 motionOnset=tmpFolder{i}.firstSample+100;
                ...
                data3{grsp}{trialCounter}=createTW2(motionEnd,min([tmpFolder{i}.finalSample,length(tmpFolder{i}.emg)]),tmpFolder{i}.emg,lengthTW,SR,MuscleSet,overlap,bandPassCuttOffFreq,lowPassCutOffFreq,normaLize,rectiFy,mvc,false,featuresIDs);

                Fdata3{grsp}{trialCounter}=createTW2(motionEnd,min([tmpFolder{i}.finalSample,length(tmpFolder{i}.emg)]),tmpFolder{i}.emg,lengthTW,SR,MuscleSet,overlap,bandPassCuttOffFreq,lowPassCutOffFreq,normaLize,rectiFy,mvc,true,featuresIDs);
...
for grsp=1:length(ClasseIDs)
    ...
    for i=1:length(SubjectData.grasp{ClasseIDs(grsp)}.trial)
        ...
        if SubjectData.grasp{ClasseIDs(grsp)}.trial{i}.motionOnset~=0
            
            [angVel,filtGonio]=preprocGonio(SubjectData.grasp{ClasseIDs(grsp)}.trial{i}.gonio,SR,cutoff_LP,order_LP,velLPCutOffFreq,order_LP);
            
            [motionOnset,motionEnd]=findMotionLimits(angVel,SR,lengthTW,overlap,0.370,velThreshold);
            
            %motionOnset=SubjectData.grasp{ClasseIDs(grsp)}.trial{i}.firstSample+100;
            testingSet{trialCounter}.emg=createTW2(motionOnset,min([SubjectData.grasp{ClasseIDs(grsp)}.trial{i}.finalSample,length(SubjectData.grasp{ClasseIDs(grsp)}.trial{i}.emg)]),SubjectData.grasp{ClasseIDs(grsp)}.trial{i}.emg,lengthTW,SR,MuscleSet,overlap,bandPassCuttOffFreq,lowPassCutOffFreq,normaLize,rectiFy,mvc,false,featuresIDs);
                    
            testingSet{trialCounter}.gonio=filtGonio(motionOnset:end);
            testingSet{trialCounter}.angularVelocity=angVel(motionOnset:end);
              
            testingSet{trialCounter}.label=grsp;
            
            
            FtestingSet{trialCounter}.emg=createTW2(motionOnset,min([SubjectData.grasp{ClasseIDs(grsp)}.trial{i}.finalSample,length(SubjectData.grasp{ClasseIDs(grsp)}.trial{i}.emg)]),SubjectData.grasp{ClasseIDs(grsp)}.trial{i}.emg,lengthTW,SR,MuscleSet,overlap,bandPassCuttOffFreq,lowPassCutOffFreq,normaLize,rectiFy,mvc,true,featuresIDs);
                    
            FtestingSet{trialCounter}.gonio=filtGonio(motionOnset:end);
            FtestingSet{trialCounter}.angularVelocity=angVel(motionOnset:end);
              
            FtestingSet{trialCounter}.label=grsp;
