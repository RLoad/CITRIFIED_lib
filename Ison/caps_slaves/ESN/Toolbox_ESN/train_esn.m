...
switch trained_esn.learningMode
    ...
    case  'offline_multipleTimeSeries' 
        ...
        for i = 1:nTimeSeries
            if strcmp(trained_esn.type, 'twi_esn')
                if size(trainInput{i,1},2) > 1
                    trained_esn.avDist = mean(sqrt(sum(((trainInput{i,1}(2:end,:) - trainInput{i,1}(1:end - 1,:))').^2)));
                else
                    trained_esn.avDist = mean(abs(trainInput{i,1}(2:end,:) - trainInput{i,1}(1:end - 1,:)));
                end
            end           
            stateCollection_i =  compute_statematrix(trainInput{i,1}, trainOutput{i,1}, trained_esn, nForgetPoints);
        ...
        collectIndex = 1;
        for i = 1:nTimeSeries
            teacherCollection_i = compute_teacher(trainOutput{i,1}, trained_esn, nForgetPoints);
