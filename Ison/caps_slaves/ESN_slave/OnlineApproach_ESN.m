...
predictedOutput=[];
if typController==1
    predictedOutput= test_esn(emgSignals, ESNmodelallmotion , nForgetPoints) ; 
else
    predictedOutput= test_esn(emgSignals, ESNmodelonlylast , nForgetPoints) ; 
end
   
disp(sum(predictedOutput))
[val,timeWindowOutput]=max(sum(predictedOutput));
